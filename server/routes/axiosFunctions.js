        ...
        if (response.status === 200) {
            ...
            for(let i = 0; i < response.data.participants.length; i++) {
                if(response.data.participants[i].teamId == blueSideCode) {
                    liveGame.bluePlayers[i] = [response.data.participants[i].summonerName, response.data.participants[i].championId];
                } else {
                    liveGame.redPlayers[i % 5] = [response.data.participants[i].summonerName, response.data.participants[i].championId];
                }
            }
